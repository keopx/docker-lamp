...
/* Without directors, only one varnish. */
backend default {
...
/* With directors. Multiple varnish*/
backend drupfe01 {
   # WARNING: timeouts could be not big enought for certain POST requests.
   .host = "web";
   .port = "80";
...
sub vcl_init {
   new fedirector = directors.round_robin();
   fedirector.add_backend(drupfe01);
   # fedirector.add_backend(drupfe02);
   return(ok);
}
...
sub vcl_recv {
  ...
  if ( req.method == "BAN" ) {
    ...
    } else if(req.http.Cache-Tags) {
      ban("obj.http.Cache-Tags ~ " + req.http.Cache-Tags);
    } else if (req.http.Host){
      ban("req.http.Host == " + req.http.Host);
    } else {
      return (synth(405, "Error adding Ban."));
    }
  ...
  # Empty in simple configs.
  set req.backend_hint = fedirector.backend();
...
sub vcl_hit {
  ...
  /* Allow varnish to serve up stale content if all backends are down */
  # See https://www.varnish-cache.org/docs/4.0/users-guide/vcl-grace.html
  # See https://www.varnish-software.com/blog/grace-varnish-4-stale-while-revalidate-semantics-varnish
  if ( ! std.healthy(req.backend_hint)
    ...
    // Object is in grace, deliver it
    // Automatically triggers a background fetch
    set req.http.X-Varnish-Grace = "extended:Sick";
    return (deliver);
  }
  /* Allow varnish to serve up stale content if a previous request failed */
  # We use req.restarts since bereq modifications made in backend process are lost when abandon is called.
  # See https://www.varnish-cache.org/docs/4.0/users-guide/vcl-grace.html
  # See https://www.varnish-software.com/blog/grace-varnish-4-stale-while-revalidate-semantics-varnish
  if ( req.restarts > 0
    && obj.ttl + obj.grace > 0s
    ...
    // Object is in grace, deliver it
    // Automatically triggers a background fetch
    set req.http.X-Varnish-Grace = "extended:Backend-Error";
    return (deliver);
  }
...
sub vcl_backend_response {
  ...
  if ( beresp.status == 500
    ...
    set bereq.http.X-Varnish-Backend-Error = "yes";
    return (retry);
  }
